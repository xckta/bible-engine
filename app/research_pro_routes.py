    ...
    if not key:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "This analysis displays canonical quotations in ESV. Add your ESV API key in Settings.",
            },
        )
...
def _codex() -> CodexClient:
    pref = preferences(settings.local_settings_path)
    return CodexClient(
        settings.codex_command,
        settings.codex_model,
        pref["reasoning_effort"],
...
def _terms(text: str) -> list[str]:
    return [
        ...
        if token not in _WORD_STOP
    ]


def _term_profile(texts: list[str], limit: int = 24) -> list[dict]:
    counts = Counter(token for text in texts for token in _terms(text))
...
def _shared_terms(left: list[str], right: list[str], limit: int = 20) -> list[dict]:
    a = Counter(token for text in left for token in _terms(text))
    b = Counter(token for text in right for token in _terms(text))
    rows = [
        (term, min(a[term], b[term]), a[term], b[term])
        for term in (a.keys() & b.keys())
    ]
    rows.sort(key=lambda row: (row[1], row[2] + row[3]), reverse=True)
    return [
        {"term": term, "shared_min": shared, "left_count": left_count, "right_count": right_count}
        for term, shared, left_count, right_count in rows[:limit]
    ]
...
def _project_context(project_id: str | None) -> str:
    ...
    pref = preferences(settings.local_settings_path)
    try:
        return build_context(settings.studies_path, project_id, pref["study_context_chars"])
...
def _worldview_evidence(data: dict, visible_limit: int) -> tuple[list[Evidence], dict]:
    ...
    for index, row in enumerate(canonical):
        evidence.append(
            Evidence(
                ...
                row["verse"],
                row["text"],
                float(-row.get("rank", 0)),
            )
        )
    for index, row in enumerate(deuterocanon):
        evidence.append(
            Evidence(
                f"WVD{index}",
                "deuterocanon",
    ...
    for index, row in enumerate(reference):
        evidence.append(
            Evidence(
                f"WVR{index}",
                "pseudepigrapha",
                row.get("source_label") or "Reference",
                row["name"],
                row.get("chapter"),
                row.get("verse_start"),
                row.get("verse_end"),
    ...
    analysis = {
        "canonical_distribution": [
            ...
            for label, count in Counter(row["book"] for row in data.get("canonical", [])).most_common()
        ],
        "reference_distribution": [
            {"label": label, "count": count}
            for label, count in Counter(row["name"] for row in data.get("reference", [])).most_common()
        ],
        "canonical_terms": _term_profile([row["text"] for row in data.get("canonical", [])]),
        "reference_terms": _term_profile([row["text"] for row in data.get("reference", [])]),
        "shared_terms": _shared_terms(
            [row["text"] for row in data.get("canonical", [])],
            [row["text"] for row in data.get("reference", [])],
        ),
    }
...
def witness_verse_dossier(
    ...
    if not readings:
        raise HTTPException(
            404,
            detail={"code": "witness_reading_missing", "message": "No installed witness edition contains that verse."},
        )

    codes = [row["code"] for row in readings]
    chosen = base if base in codes else codes[0]
    base_row = next(row for row in readings if row["code"] == chosen)
    comparisons = []
    for row in readings:
        if row["code"] == chosen:
            continue
        comparisons.append(
            {"code": row["code"], "name": row["name"], **collate_texts(base_row["text"], row["text"])}
        )

    pairwise = []
    for index, left in enumerate(readings):
        for right in readings[index + 1 :]:
            comparison = collate_texts(left["text"], right["text"])
            pairwise.append(
                {
                    "left": left["code"],
                    "right": right["code"],
                    "similarity": comparison["similarity"],
                    "changed_tokens": comparison["changed_tokens"],
                }
            )
...
def witness_chapter_scan(
    ...
    with session(settings.db_path) as conn:
        left_rows = {
            int(row["verse"]): row["text"]
            for row in conn.execute(
                "SELECT verse,text FROM textual_verses WHERE edition_code=? AND book=? AND chapter=? ORDER BY verse",
                (left, normalized, chapter),
            ).fetchall()
        }
        right_rows = {
            int(row["verse"]): row["text"]
            for row in conn.execute(
                "SELECT verse,text FROM textual_verses WHERE edition_code=? AND book=? AND chapter=? ORDER BY verse",
                (right, normalized, chapter),
            ).fetchall()
        }
    if not left_rows and not right_rows:
        raise HTTPException(
            404,
            detail={"code": "witness_chapter_missing", "message": "Neither selected edition contains that chapter."},
        )

    rows: list[dict] = []
    identical = variants = missing = 0
    verse_union = sorted(set(left_rows) | set(right_rows))
    for verse_number in verse_union:
        left_text = left_rows.get(verse_number)
        right_text = right_rows.get(verse_number)
        if left_text is None or right_text is None:
            missing += 1
            row = {
                "verse": verse_number,
                "reference": f"{normalized} {chapter}:{verse_number}",
                "status": "missing",
                "left": left_text or "",
                "right": right_text or "",
                "similarity": 0.0,
            ...
            comparison = collate_texts(left_text, right_text)
            same = comparison["changed_tokens"] == 0
            identical += int(same)
            variants += int(not same)
            row = {
                "verse": verse_number,
                "reference": f"{normalized} {chapter}:{verse_number}",
                "status": "equal" if same else "variant",
                "left": left_text,
                "right": right_text,
                **comparison,
            }
    ...
    return {
        ...
        "left": left,
        "right": right,
        "stats": {"verse_union": len(verse_union), "identical": identical, "variants": variants, "missing": missing},
        "rows": rows,
...
def worldview_dossier(req: WorldviewRequest):
    ...
    if any(item.tier == "canonical" for item in evidence):
        try:
    ...
    if req.synthesize and evidence:
        try:
            context = (
                "HISTORICAL LENS (organizing metadata, not evidence): "
                f"{data['period']['name']} // {data['period']['range']} // {data['period']['note']}"
            )
            ...
            synthesis = answer_question(
                (
                    "Within this research lens, identify what the supplied texts establish about: "
                    f"{req.query}. Distinguish canonical claims from ancient reference parallels and do not infer "
                    "literary dependence from thematic similarity alone."
                ),
                evidence,
                _codex(),
    ...
    return {
        ...
        "query": req.query,
        "analysis": analysis,
        "evidence": [item.__dict__ | {"citation": item.citation} for item in evidence],
        "synthesis": synthesis,
...
def _parse_biblical_reference(ref: str) -> tuple[str, int | None, int | None]:
    ...
    return (
        ...
        if match
        else (ref.strip(), None, None)
    )


def _reference_work_name(reference: str, fallback: str) -> str:
    if not reference:
        return fallback
    return re.sub(r"\s+\d+(?::\d+(?:[–-]\d+)?)?$", "", reference).strip() or fallback
...
def traditions_dossier(req: TraditionsRequest):
    ...
    canonical_hits = [
        ...
        if hit["tier"] == "canonical"
    ]
    if canonical_hits:
        try:
            passages = _esv().fetch_many([hit["reference"] for hit in canonical_hits])
        except ESVError as exc:
            raise HTTPException(502, detail={"code": "esv_error", "message": str(exc)}) from exc
    ...
    for column in matrix["columns"]:
        group_text[column["name"]] = [hit["text"] for hit in column["hits"]]
        for hit in column["hits"]:
            if hit["tier"] in {"canonical", "deuterocanon"}:
                ...
                work = _reference_work_name(hit.get("reference", ""), column["name"])
                evidence.append(
                    Evidence(
                        f"TM-{len(evidence)}",
                        "pseudepigrapha",
                        hit.get("source_label") or "Reference",
                        work,
                        None,
                        None,
                        None,
    ...
    for index, left_name in enumerate(names):
        for right_name in names[index + 1 :]:
            left_terms = set(_terms(" ".join(group_text[left_name])))
            right_terms = set(_terms(" ".join(group_text[right_name])))
    ...
    if req.synthesize and evidence:
        try:
            ...
            synthesis = answer_question(
                (
                    "Compare the supplied textual evidence for this theme: "
                    f"{req.query}. State what is explicit in canonical Scripture, what appears in "
                    "deuterocanonical/reference literature, and where similarity is only thematic rather than "
    ...
    return {
        "matrix": matrix,
        "overlaps": sorted(overlaps, key=lambda row: row["jaccard"], reverse=True),
        "synthesis": synthesis,
        "synthesis_error": synthesis_error,
...
def _vault_source_payload(source_id: str, offset: int = 0, limit: int = 80) -> dict:
    with session(settings.db_path) as conn:
        ...
        total = int(
            conn.execute("SELECT COUNT(*) n FROM vault_chunks WHERE source_id=?", (source_id,)).fetchone()["n"]
        )
        chunks = [
            dict(row)
            for row in conn.execute(
                "SELECT id,ordinal,text FROM vault_chunks WHERE source_id=? ORDER BY ordinal LIMIT ? OFFSET ?",
                (source_id, limit, offset),
            ).fetchall()
        ]
    row = dict(source)
    row["source_class_label"] = CLASSES.get(row["source_class"], row["source_class"])
...
def vault_source(
    ...
    return _vault_source_payload(source_id, offset, limit)
...
def vault_update(source_id: str, req: VaultUpdateRequest):
    ...
    if not updates:
        return _vault_source_payload(source_id)
    with session(settings.db_path) as conn:
        ensure_vault_schema(conn)
        ...
        conn.execute(
            f"UPDATE vault_sources SET {columns} WHERE id=?",
...
class ProDeepDiveRequest(BaseModel):
    ...
    vault_classes: list[
        Literal["primary_ancient", "lexicon", "scholarship", "personal_notes", "other"]
...
def _vault_evidence(hit: dict) -> Evidence:
    ...
    return Evidence(
        ...
        hit["text"],
        float(hit.get("score", 0)),
        source_label=hit.get("source_class_label") or "User Vault",
    )
...
def pro_deep_dive(req: ProDeepDiveRequest):
    ...
    with session(settings.db_path) as conn:
        for item in plan.questions:
            rows = retrieve(
                conn,
                item.question,
                min(14, pref["top_k_canonical"] + 6),
                min(18, pref["top_k_reference"] + 10),
                True,
                True,
            )
            vault_rows = (
                search_vault(conn, item.question, 6, req.vault_classes)
                if req.include_vault
                else []
            )
            all_rows = rows + [_vault_evidence(hit) for hit in vault_rows]
            packets.append(
                {
                    "question": item.question,
                    "purpose": item.purpose,
                    "shelf_focus": item.shelf_focus,
                    "counts": dict(Counter(row.tier for row in all_rows)),
                    "citations": [row.citation for row in all_rows],
                }
            )
            for item_evidence in all_rows:
                merged[(item_evidence.tier, item_evidence.citation, item_evidence.text[:140])] = item_evidence

        if req.include_vault:
            for hit in search_vault(conn, req.question, 10, req.vault_classes):
                item_evidence = _vault_evidence(hit)
                merged[(item_evidence.tier, item_evidence.citation, item_evidence.text[:140])] = item_evidence

    priority = {"canonical": 0, "deuterocanon": 1, "pseudepigrapha": 2, "vault": 3}
    evidence = sorted(
        merged.values(),
        key=lambda item: (priority.get(item.tier, 9), -item.score),
    )[: req.max_evidence]
    ...
    payload = {
        ...
        "tier_counts": dict(Counter(item.tier for item in evidence)),
        "vault_included": req.include_vault,
    ...
    if req.project_id and result.mode == "codex_closed_corpus":
        try:
            append_consultation(
                settings.studies_path,
                req.project_id,
                "DEEP DIVE PRO: " + req.question,
                result.__dict__,
            )
        except Exception:
            pass

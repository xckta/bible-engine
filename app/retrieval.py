...
from .providers import OllamaClient, ProviderError, hashed_embedding
...
STOPWORDS = {"the","and","for","that","this","with","from","what","does","say","says","about","into","were","was","are","who","why","how","which","where","when","bible","scripture","verse","verses","compare","tell","me","can","could","would","should"}

def lexical_search(conn: sqlite3.Connection, query: str, translations: list[str], limit: int) -> list[Passage]:
    terms = [t for t in re.findall(r"[A-Za-z0-9']+", query.lower()) if len(t) > 2 and t not in STOPWORDS]
    if not terms:
        return []
    ...
    return [Passage(id=r["id"], translation=r["translation"], book=r["book"], chapter=r["chapter"], verse=r["verse"], text=r["text"], score=float(-r["score"])) for r in rows]
...
def semantic_search(conn: sqlite3.Connection, query: str, translations: list[str], limit: int, ollama: OllamaClient | None) -> list[Passage]:
    if ollama:
        try:
            qv = ollama.embed([query])[0]
        ...
        model = "hash-384"
    clause, params = _translation_clause(translations)
    # Semantic search is intentionally index-backed. If embeddings have not been built,
    # lexical retrieval remains available instead of recomputing every verse on each query.
    rows = conn.execute(
    ...
    for r in rows:
        ...
        score = cosine(qv, vec)
        if score > 0:
            scored.append(Passage(id=r["id"], translation=r["translation"], book=r["book"], chapter=r["chapter"], verse=r["verse"], text=r["text"], score=score))
...
def expand_context(conn: sqlite3.Connection, passages: list[Passage], radius: int) -> list[Passage]:
    ...
    for p in passages:
        rows = conn.execute(
            "SELECT v.id,t.code translation,v.book,v.chapter,v.verse,v.text FROM verses v JOIN translations t ON t.id=v.translation_id "
...
def retrieve(conn: sqlite3.Connection, query: str, translations: list[str], top_k: int, radius: int, semantic: bool, ollama: OllamaClient | None) -> list[Passage]:
    ...
    semantic_hits = semantic_search(conn, query, translations, top_k, ollama) if semantic else []
    merged: dict[int, Passage] = {}
    for p in lexical + semantic_hits:
        prev = merged.get(p.id)
        if not prev or p.score > prev.score:
            merged[p.id] = p
    return expand_context(conn, sorted(merged.values(), key=lambda x: x.score, reverse=True)[:top_k], radius)

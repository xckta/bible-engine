...
def replace_original_words(conn: sqlite3.Connection, source_id: int, words: Iterable[dict], batch_size: int = 5000) -> int:
    ...
    for w in words:
        batch.append((
            ...
            w.get("source_word_id", ""), w["surface"], w.get("surface_normalized", ""), w.get("lemma", ""),
            w.get("lemma_normalized", ""), w.get("alt_lemma", ""), w.get("strongs", ""), w.get("morph", ""),
            w.get("morph_expanded", ""), w.get("transliteration", ""), w.get("word_language", ""),
            w.get("source_book", w["book"]), int(w.get("source_chapter", w["chapter"])),
            int(w.get("source_verse", w["verse"])), w.get("verse_mapping_type", "same"),
        ))
...
def replace_original_lexicon(conn: sqlite3.Connection, language: str, entries: Iterable[dict]) -> int:
    """Replace the historical Strong's layer for one language.

    Supplemental BDB/OpenScriptures fields are intentionally preserved for matching
    Strong's rows so re-running the Strong's importer cannot destroy richer lexical data.
    """
    ensure_original_schema(conn)
    prefix = "H" if language.lower() == "hebrew" else "G"
    existing = {
        r["strongs"]: dict(r)
        for r in conn.execute("SELECT * FROM original_lab_lexicon WHERE strongs LIKE ?", (prefix + "%",)).fetchall()
    }
    ...
    for e in entries:
        ...
        rows.append((
            ...
            e.get("source_url", ""), e.get("license", ""), old.get("gloss", ""), old.get("pos", ""),
            old.get("transliteration", ""), old.get("bdb_ref", ""), old.get("twot_ref", ""), old.get("etymology", ""),
            old.get("semantic_range", ""), old.get("lexical_source_label", ""), old.get("lexical_source_url", ""),
            old.get("lexical_license", ""),
        ))
    conn.executemany(
        ...
        "bdb_ref,twot_ref,etymology,semantic_range,lexical_source_label,lexical_source_url,lexical_license) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        rows,
    )
    return len(rows)
...
def merge_original_lexicon_profiles(conn: sqlite3.Connection, entries: Iterable[dict]) -> int:
    """Merge BDB/OpenScriptures lexical metadata onto Strong's-keyed rows."""
    ensure_original_schema(conn)
    count = 0
    for e in entries:
        ...
        conn.execute(
            "UPDATE original_lab_lexicon SET "
            "headword=CASE WHEN headword='' THEN ? ELSE headword END,"
...
def replace_lxx_lemma_occurrences(conn: sqlite3.Connection, entries: Iterable[dict], batch_size: int = 10000) -> int:
    ...
    for e in entries:
        batch.append((
            ...
            e.get("source_label", "Open Scriptures Septuagint Project — lemma index"),
            e.get("source_url", "https://github.com/openscriptures/GreekResources"),
        ))
        if len(batch) >= batch_size:
            conn.executemany(sql, batch)
            total += len(batch)
...
def replace_original_verse_mappings(conn: sqlite3.Connection, source_code: str, mappings: Iterable[dict]) -> int:
    ...
    conn.executemany(
        "INSERT INTO original_lab_verse_mappings(source_code,source_book,source_chapter,source_verse,source_segment,target_book,target_chapter,target_verse,target_segment,mapping_type) "
        "VALUES(?,?,?,?,?,?,?,?,?,?)", rows
    )
    return len(rows)
